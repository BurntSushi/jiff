...
fn copy_rust_source_file(src: &Path, dst: &Path) -> anyhow::Result<()> {
    ...
    let code =
        remove_only_jiffs(&remove_defmt(&remove_cfg_alloc_or_std(&code)));
...
/// Removes all `#[cfg(feature = "defmt")]` gates and gated items.
///
/// The `jiff-static` proc macro only needs these shared types for compile-time
/// TZif/POSIX parsing and code generation. It never formats them with defmt.
fn remove_defmt(code: &str) -> String {
    static CFG_ATTR_RE: LazyLock<Regex> = LazyLock::new(|| {
        Regex::new(r###"#\[cfg_attr\(feature = "defmt", .*\n"###).unwrap()
    });
    static CFG_ITEM_RE: LazyLock<Regex> = LazyLock::new(|| {
        Regex::new(
            r###"(?m:^)#\[cfg\(feature = "defmt"\)\]\n.*\{\n[\s\S]*?\n}\n"###,
        )
        .unwrap()
    });
    CFG_ITEM_RE
        .replace_all(&CFG_ATTR_RE.replace_all(code, ""), "")
        .into_owned()
}
