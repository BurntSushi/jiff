...
fn copy(srcdir: &Path, dstdir: &Path, dir: &Path) -> anyhow::Result<()> {
    ...
    // We have to do rustfmt after all copying is done, otherwise rustfmt
    // might fail on the incomplete module?
    for result in walkdir::WalkDir::new(dstdir.join(dir)) {
        let dent = result?;
        if dent.file_type().is_file() && dent.path().ends_with(".rs") {
            super::rustfmt(&dent.path())?;
        }
    }
    Ok(())
}
