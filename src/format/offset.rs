...
use crate::{
    ...
    format::{temporal, util::DecimalFormatter, Parsed},
    tz::Offset,
    util::{
        escape, parse,
        rangeint::{ri8, RFrom},
...
/// local tim eoffset), or a specific numeric offset given in hours, minutes,
/// seconds and nanoseconds (with everything except hours being optional).
#[derive(Debug)]
pub(crate) struct ParsedOffset<'i> {
    /// The original input that the offset was parsed from.
    input: escape::Bytes<'i>,
    /// The kind of offset parsed.
    kind: ParsedOffsetKind,
}

impl<'i> ParsedOffset<'i> {
...
enum ParsedOffsetKind {
    ...
    /// local time is unknown or unavailable.
    Zulu,
...
struct Numeric {
    ...
    nanoseconds: Option<t::FractionalNanosecond>,
}
...
impl Numeric {
    ...
    fn to_offset(&self) -> Result<Offset, Error> {
        ...
        Ok(Offset::new_seconds_ranged(seconds * self.sign))
    }
}
...
impl Parser {
    ...
    /// indicator for UTC when the local time offset is unknown or unavailable.
    ///
    ...
    pub(crate) fn parse<'i>(
        &self,
        mut input: &'i [u8],
    ) -> Result<Parsed<'i, ParsedOffset<'i>>, Error> {
        if input.is_empty() {
        ...
        if input[0] == b'Z' || input[0] == b'z' {
            ...
            input = &input[1..];
            let value = ParsedOffset {
                input: escape::Bytes(mkslice(input)),
                kind: ParsedOffsetKind::Zulu,
            };
        ...
        let value = ParsedOffset {
            input: escape::Bytes(mkslice(input)),
            kind: ParsedOffsetKind::Numeric(numeric),
        };
    ...
    fn parse_numeric<'i>(
        ...
        let Parsed { value: sign, input } =
            self.parse_sign(input).map_err(|err| {
                err.context(err!(
                    "failed to parse sign in UTC numeric offset {original:?}"
                ))
            })?;

        // Parse hours component.
        let Parsed { value: hours, input } =
            self.parse_hours(input).map_err(|err| {
                err.context(err!(
        ...
        let Parsed { value: has_minutes, input } =
            self.parse_separator(input, extended).map_err(|err| {
                err.context(err!(
        ...
        let Parsed { value: minutes, input } =
            self.parse_minutes(input, extended).map_err(|err| {
                err.context(err!(
        ...
        let Parsed { value: seconds, input } =
            self.parse_seconds(input, extended).map_err(|err| {
                err.context(err!(
        ...
        let Parsed { value: nanoseconds, input } =
            temporal::parse_fractional_nanosecond(input).map_err(|err| {
                err.context(err!(
    ...
    fn parse_hours<'i>(
        ...
        let hours = parse::i64(hours).map_err(|err| {
            err!(
        ...
        let hours = ParsedOffsetHours::try_new("hours", hours)
            .map_err(|err| err!("hours are not valid: {err}"))?;
    ...
    fn parse_minutes<'i>(
        ...
        let minutes = parse::i64(minutes).map_err(|err| {
            err!(
                "failed to parse {minutes:?} as minutes \
                 (a two digit integer): {err}",
                minutes = escape::Bytes(minutes),
            )
        })?;
        let minutes = ParsedOffsetMinutes::try_new("minutes", minutes)
            .map_err(|err| err!("minutes are not valid: {err}",))?;
    ...
    fn parse_seconds<'i>(
        ...
        let seconds = parse::i64(seconds).map_err(|err| {
            err!(
                "failed to parse {seconds:?} as seconds \
                 (a two digit integer): {err}",
                seconds = escape::Bytes(seconds),
            )
        })?;
        let seconds = ParsedOffsetSeconds::try_new("seconds", seconds)
            .map_err(|err| err!("seconds are not valid: {err}",))?;
...
mod tests {
    ...
    fn err_numeric_hours_out_of_range() {
        insta::assert_snapshot!(
            ...
            @r###"failed to parse hours in UTC numeric offset "-26": hours are not valid: parameter 'hours' with value 26 is not in the required range of 0..=25"###,
        );
    }
    ...
    fn err_numeric_seconds_out_of_range() {
        insta::assert_snapshot!(
            ...
            @r###"failed to parse seconds in UTC numeric offset "-05:30:60": seconds are not valid: parameter 'seconds' with value 60 is not in the required range of 0..=59"###,
        );
    }
    ...
    fn err_numeric_too_big_for_offset() {
        ...
        insta::assert_snapshot!(
            ...
            @"due to precision loss, UTC offset '+25:59:59.5' \
              is rounded to a value that is out of bounds",
        );
    }
