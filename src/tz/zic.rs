...
WIP: I had dreams of providing a time zone database that could be backed by
...
going to involve a more overhead than what Tzif does. And if we were to try
...
struct Zone {
    ...
    format: ZoneFormatP,
    /// The instant until which this zone is active (exclusive).
...
impl FromStr for RuleAtP {
    ...
    fn from_str(at: &str) -> Result<RuleAtP, Error> {
        ...
        let (duration_string, suffix_string) = at.split_at(at.len() - 1);
        if suffix_string.chars().all(|ch| ch.is_ascii_alphabetic()) {
            ...
            Ok(RuleAtP { duration, suffix: Some(suffix) })
        } else {
            let duration = parse_duration(at)?;
            Ok(RuleAtP { duration, suffix: None })
        }
    }
}
...
impl RuleSaveP {
    ...
    ///
    /// If the duration is too long to fit in an offset, then an error is
    /// returned.
    fn to_offset(&self) -> Result<Offset, Error> {
        ...
        let seconds =
            i32::try_from(self.duration.seconds()).map_err(|_| {
                Error::signed(
                    "SAVE seconds",
...
impl FromStr for RuleSaveP {
    ...
    fn from_str(at: &str) -> Result<RuleSaveP, Error> {
        ...
        if suffix_string.chars().all(|ch| ch.is_ascii_alphabetic()) {
            let duration = parse_duration(duration_string)?;
            let suffix = suffix_string.parse()?;
            Ok(RuleSaveP { duration, suffix: Some(suffix) })
        } else {
            let duration = parse_duration(at)?;
            Ok(RuleSaveP { duration, suffix: None })
        }
    }
}
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
struct ZoneStdoffP {
    /// The duration in 96-bit nanoseconds.
    duration: TimeDuration,
}
...
impl FromStr for ZoneStdoffP {
    ...
    fn from_str(stdoff: &str) -> Result<ZoneStdoffP, Error> {
        let duration = parse_duration(stdoff)?;
        Ok(ZoneStdoffP { duration })
    }
}
...
enum ZoneUntilP {
    ...
    YearMonthDayTime {
        ...
        month: RuleInP,
        day: RuleOnP,
        /// Note that adding a time duration to the year/month/day could
        /// overflow the allowed maximum time. This isn't handled until this
        /// type is converted into higher level data types.
        duration: RuleAtP,
    },
}
...
impl ZoneUntilP {
    ...
    fn to_datetime(&self) -> Result<DateTime, Error> {
        ...
        let dt = date
            ...
            .checked_add(self.at().duration.to_span())?;
        Ok(dt)
    }
    ...
    fn at(&self) -> RuleAtP {
        ...
        match *self {
            Year { .. } | YearMonth { .. } | YearMonthDay { .. } => {
                RuleAtP { duration: TimeDuration::ZERO, suffix: None }
            }
...
fn parse_duration(duration: &str) -> Result<TimeDuration, Error> {
    ...
    let span_to_duration = |span: Span| -> Result<TimeDuration, Error> {
        span.only_time_parts_to_duration()
            .map_err(|_| err!("time duration {duration:?} is too big"))
    };
    ...
    fn read_next_fields(&mut self) -> Result<bool, Error> {
        ...
        loop {
            ...
            parse_fields(&line, &mut self.fields)
                .map_err(|e| e.context(err!("line {}", self.line_number)))?;
...
mod tests {
    ...
    fn td(seconds: i64, nanoseconds: i32) -> TimeDuration {
        TimeDuration::try_new(seconds, nanoseconds).unwrap()
    }
    ...
    fn zone_until_to_datetime() {
        ...
        assert_eq!(wall, DateTime::constant(2024, 1, 1, 0, 0, 0, 0));

        let until = ZoneUntilP::parse(&["2024", "Mar"]).unwrap();
        let wall = until.to_datetime().unwrap();
        assert_eq!(wall, DateTime::constant(2024, 3, 1, 0, 0, 0, 0));

        let until = ZoneUntilP::parse(&["2024", "Mar", "Sun>=8"]).unwrap();
        let wall = until.to_datetime().unwrap();
        assert_eq!(wall, DateTime::constant(2024, 3, 10, 0, 0, 0, 0));

        let until =
            ZoneUntilP::parse(&["2024", "Mar", "Sun>=8", "2:00"]).unwrap();
        let wall = until.to_datetime().unwrap();
        assert_eq!(wall, DateTime::constant(2024, 3, 10, 2, 0, 0, 0));
    ...
    fn parse_rule_at_ok() {
        ...
        assert_eq!(at, RuleAtP { duration: td(5 * 60 * 60, 0), suffix: None });

        let at: RuleAtP = "5w".parse().unwrap();
        assert_eq!(
            at,
            RuleAtP {
                duration: td(5 * 60 * 60, 0),
        ...
        assert_eq!(at, RuleAtP { duration: td(0, 0), suffix: None });
        ...
        assert_eq!(
            ...
            RuleAtP {
                duration: td(0, 0),
                suffix: Some(RuleAtSuffixP::Standard)
            }
        );
    }
    ...
    fn parse_rule_save_ok() {
        ...
        assert_eq!(
            at,
            RuleSaveP { duration: td(5 * 60 * 60, 0), suffix: None }
        );
        ...
        assert_eq!(at, RuleSaveP { duration: td(0, 0), suffix: None });
    ...
    fn parse_zone_stdoff_ok() {
        ...
        assert_eq!(stdoff, ZoneStdoffP { duration: td(5 * 60 * 60, 0) });
    ...
    fn parse_zone_until_ok() {
        ...
        assert_eq!(
            ...
            ZoneUntilP::YearMonthDayTime {
                ...
                month: RuleInP { month: t::Month::new(1).unwrap() },
                day: RuleOnP::Last { weekday: Weekday::Sunday },
                duration: RuleAtP { duration: td(0, 0), suffix: None },
            },
        );
        let until =
        ...
        assert_eq!(
            ...
            ZoneUntilP::YearMonthDayTime {
                ...
                month: RuleInP { month: t::Month::new(1).unwrap() },
                day: RuleOnP::Last { weekday: Weekday::Sunday },
                duration: RuleAtP { duration: td(3661, 1), suffix: None },
            },
        );
        let until =
    ...
    fn parse_duration_err() {
        ...
        // Tricky cases where the number of hours is OK, but adding another
        // second to it trips the boundary condition.
        assert!(parse_duration("175307616").is_ok());
        assert!(parse_duration("175307617").is_err());
        assert!(parse_duration("175307616:01").is_err());
        assert!(parse_duration("175307616:00:01").is_err());
        // OK because it fills up the nanosecond component of the duration and
        // doesn't increment the number of seconds beyond what is supported.
