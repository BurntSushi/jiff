...
this module also plays a supporting role for `TZ` strings in the IANA `tzfile`
...
use crate::{
    ...
    span::Span,
    tz::{AmbiguousInstantKind, Dst, Offset},
    util::{
        escape::{Byte, Bytes},
        parse,
...
impl ReasonablePosixTimeZone {
    ...
    pub(crate) fn to_ambiguous_instant_kind(
        ...
        let Some(dst_info) = self.dst_info_wall(year) else {
            return AmbiguousInstantKind::Unambiguous { offset: std_offset };
        };
        ...
        if diff.get_seconds_ranged() == 0 {
            ...
            AmbiguousInstantKind::Unambiguous { offset: std_offset }
        } else if diff.is_negative() {
            // For DST transitions that always move behind one hour, ambiguous
            // instants only occur when the given civil datetime falls in the
            // standard time range.
            if dst_info.in_dst(dt) {
                AmbiguousInstantKind::Unambiguous { offset: dst_info.offset }
            } else {
            ...
            if !dst_info.in_dst(dt) {
                ...
                if dst_info.start <= dt && dt < gap_end {
                    ...
                    AmbiguousInstantKind::Unambiguous {
    ...
    /// Returns the offset for daylight savings time in this POSIX time zone.
    ///
    ...
    /// whether an instant (a timestamp in UTC) is in DST or not.
...
/// The daylight savings time (DST) info for a POSIX time zone in a particular
/// year.
#[derive(Debug, Eq, PartialEq)]
struct DstInfo<'a> {
...
/// The daylight-savings-time abbreviation, offset and rule for this time zone.
#[derive(Debug, Eq, PartialEq)]
struct PosixDst {
    abbrev: Box<str>,
    offset: Option<PosixOffset>,
...
/// The offset from UTC for standard-time or daylight-savings-time for this
/// time zone.
...
impl PosixOffset {
    ...
    fn to_offset(&self) -> Offset {
        ...
        let seconds = (hour * t::SECONDS_PER_HOUR)
            ...
            + second;
        Offset::new_seconds_ranged(sign * seconds)
    }
    ...
    fn to_civil_date(&self, year: impl RInto<Year>) -> Option<Date> {
        match *self {
            PosixDateSpec::JulianOne(day) => {
                ...
                Some(
                    first
                        .with_day_of_year_no_leap(day.get())
            ...
            PosixDateSpec::JulianZero(day) => {
                ...
                first.with_day_of_year(off1).ok()
            }
...
mod tests {
    ...
    fn reasonable_to_dst_civil_datetime_utc_range() {
        ...
        let dst_info = DstInfo {
            ...
            offset: crate::tz::Offset::constant(-3),
            start: DateTime::constant(2024, 1, 1, 1, 0, 0, 0),
            end: DateTime::constant(2024, 12, 31, 23, 0, 0, 0),
        };
        ...
        let dst_info = DstInfo {
            dst: tz.dst.as_ref().unwrap(),
            offset: crate::tz::Offset::constant(-3),
            start: DateTime::constant(2024, 1, 1, 0, 0, 0, 0),
            end: DateTime::constant(2024, 12, 31, 23, 59, 59, 999_999_999),
        };
        assert_eq!(tz.dst_info_utc(C(2024)), Some(dst_info));
        ...
        let dst_info = DstInfo {
            dst: tz.dst.as_ref().unwrap(),
            offset: crate::tz::Offset::constant(-4),
            start: DateTime::constant(2024, 3, 10, 7, 0, 0, 0),
            end: DateTime::constant(2024, 11, 3, 6, 0, 0, 0),
        };
        assert_eq!(tz.dst_info_utc(C(2024)), Some(dst_info));
    }
    ...
    fn posix_date_time_spec_to_datetime() {
        ...
        assert_eq!(
            to_datetime(&tz.rule().end, 2024),
            DateTime::constant(2024, 12, 31, 23, 0, 0, 0),
        );

        let tz = reasonable_posix_time_zone("XXX3EDT4,0/0,365");
        assert_eq!(
            to_datetime(&tz.rule().end, 2023),
            DateTime::constant(2023, 12, 31, 23, 59, 59, 999_999_999),
        );
        assert_eq!(
            to_datetime(&tz.rule().end, 2024),
            DateTime::constant(2024, 12, 31, 2, 0, 0, 0),
    ...
    fn posix_date_spec_to_date() {
        ...
        assert_eq!(start, Some(Date::constant(2023, 3, 12)));
        let end = tz.rule().end.date.to_civil_date(C(2023));
        assert_eq!(end, Some(Date::constant(2023, 11, 5)));
        let start = tz.rule().start.date.to_civil_date(C(2024));
        assert_eq!(start, Some(Date::constant(2024, 3, 10)));
        let end = tz.rule().end.date.to_civil_date(C(2024));
        assert_eq!(end, Some(Date::constant(2024, 11, 3)));

        let tz = reasonable_posix_time_zone("EST+5EDT,J60,J365");
        let start = tz.rule().start.date.to_civil_date(C(2023));
        assert_eq!(start, Some(Date::constant(2023, 3, 1)));
        let end = tz.rule().end.date.to_civil_date(C(2023));
        assert_eq!(end, Some(Date::constant(2023, 12, 31)));
        let start = tz.rule().start.date.to_civil_date(C(2024));
        assert_eq!(start, Some(Date::constant(2024, 3, 1)));
        let end = tz.rule().end.date.to_civil_date(C(2024));
        assert_eq!(end, Some(Date::constant(2024, 12, 31)));
        ...
        assert_eq!(end, None);
        let start = tz.rule().start.date.to_civil_date(C(2024));
        assert_eq!(start, Some(Date::constant(2024, 2, 29)));
        let end = tz.rule().end.date.to_civil_date(C(2024));
        assert_eq!(end, Some(Date::constant(2024, 12, 31)));

        let tz = reasonable_posix_time_zone("EST+5EDT,M1.1.1,M12.5.2");
        let start = tz.rule().start.date.to_civil_date(C(2024));
        assert_eq!(start, Some(Date::constant(2024, 1, 1)));
        let end = tz.rule().end.date.to_civil_date(C(2024));
        assert_eq!(end, Some(Date::constant(2024, 12, 31)));
    }
    ...
    fn posix_time_spec_to_civil_time() {
        ...
        assert_eq!(
            ...
            Some(Time::constant(2, 0, 0, 0)),
        );
        assert_eq!(
            tz.dst.as_ref().unwrap().rule.end.time().to_civil_time(),
            Some(Time::constant(5, 12, 34, 0)),
        );
