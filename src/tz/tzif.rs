...
use crate::{
    ...
    tz::{
        ...
        AmbiguousInstantKind, Dst, Offset,
    },
...
impl Tzif {
    ...
    /// Returns the appropriate time zone offset to use for the given instant.
    ///
    ...
    pub(crate) fn to_offset<S: TimeScale>(
        &self,
        instant: Instant<S>,
    ) -> (Offset, Dst, &str) {
        // TZif data defines transitions using Unix time, so we always need
        // an instant using the Unix time scale. This _should_ optimize to a
        // no-op when `S = Unix`.
        let instant = instant.to_unix_scale();
        ...
        // `Instant` value. TZif doesn't do this because there is no universal
        // minimum timestamp. (`i64::MIN` is a candidate, but that's likely to
        // cause overflow in readers that don't do error checking.)
        //
        ...
        let search =
            self.transitions.binary_search_by_key(&instant, |t| t.instant);
        let index = match search {
            // Since the first transition is always Instant::MIN, it's
            // impossible for any instant to sort before it.
            Err(0) => unreachable!("impossible to come before Instant::MIN"),
            Ok(i) => i,
            // i points to the position immediately after the matching instant.
            // And since we know that i>0 because of the i==0 check above, we
            // can safely subtract 1.
        ...
        let t = if index < self.transitions.len() - 1 {
            ...
            match self.posix_tz.as_ref() {
                ...
                Some(tz) => return tz.to_offset(instant),
    ...
    /// Returns a possibly ambiguous instant for the given civil datetime.
    ///
    /// The given datetime should correspond to the "wall" clock time of what
    /// humans use to tell time for this time zone.
    ///
    /// Note that "ambiguous instant" is represented by the possible selection
    /// of offsets that could be applied to the given datetime. In general, it
    /// is only ambiguous around transitions to-and-from DST. The ambiguity can
    /// arise as a "fold" (when a particular wall clock time is repeated) or as
    /// a "gap" (when a particular wall clock time is skipped entirely).
    pub(crate) fn to_ambiguous_instant_kind(
        &self,
        dt: DateTime,
        ...
        // the given datetime. Except, we do it on wall clock times instead of
        // instants. And in particular, each transition begins with a possibly
        // ambiguous range of wall clock times corresponding to either a "gap"
        // or "fold" in time.
        assert!(!self.transitions.is_empty(), "transitions is non-empty");
        let search =
        ...
        match this.wall {
            TransitionWall::Gap { end, .. } if dt < end => {
                ...
                return AmbiguousInstantKind::Gap {
                    before: prev_offset,
                    after: this_offset,
                };
            }
            TransitionWall::Fold { end, .. } if dt < end => {
                ...
                let prev = &self.transitions[prev_index];
                let prev_offset = self.local_time_type(prev).offset;
                return AmbiguousInstantKind::Fold {
                    before: prev_offset,
                    after: this_offset,
                };
            }
        ...
        if this_index == self.transitions.len() - 1 {
            if let Some(tz) = self.posix_tz.as_ref() {
                return tz.to_ambiguous_instant_kind(dt);
            }
        ...
        AmbiguousInstantKind::Unambiguous { offset: this_offset }
    }
    ...
    fn first_transition(&self) -> &Transition {
        ...
        // transition corresponding to the minimum possible Jiff instant. This
        // makes some logic for transition lookups a little simpler by reducing
        // special cases.
        self.transitions.first().unwrap()
    }

    fn first_local_time_type(&self) -> &LocalTimeType {
        // OK because TZif data with 0 local time types is invalid. So
        // we're guaranteed to have at least one.
    ...
    fn parse64(
        ...
        // transition (accounting for the dummy `Instant::MIN` transition). So
        // if we only have 1 transition and a POSIX TZ string, then we should
        // not validate it since it's equivalent to the case of 0 transitions
        // and a POSIX TZ string.
        if tzif.transitions.len() > 1 {
            if let Some(ref tz) = tzif.posix_tz {
                ...
                let (offset, dst, abbrev) = tz.to_offset(last.instant);
    ...
    fn parse_transitions<'b>(
        ...
        // Similarly, since this is the minimum instant value, it will trigger
        // for any times before the first transition found in the TZif data.
        ...
        while let Some(chunk) = it.next() {
            ...
            let instant = Instant::from_unix(seconds, 0).map_err(|e| {
    ...
    fn parse_transition_types<'b>(
        ...
        // dummy first transition corresponding to `Instant::MIN`. Its type
    ...
    fn parse_local_time_types<'b>(
        ...
        while let Some(chunk) = it.next() {
            ...
            let offset = Offset::new_seconds(offset_seconds).map_err(|e| {
                err!("found local time type with out-of-bounds offset: {e}")
            })?;
    ...
    fn parse_leap_seconds<'b>(
        ...
        while let Some(chunk) = it.next() {
            ...
            let occurrence =
                Instant::from_unix(occur_seconds, 0).map_err(|e| {
                    err!(
    ...
    /// each transition. This is used to resolve ambiguous instants when given
    /// a civil datetime.
    fn set_wall_datetimes(&mut self) {
        ...
        for i in 0..self.transitions.len() {
            ...
            t.wall = if prev == this {
                ...
                let start = prev.to_datetime(t.instant);
                ...
                let start = prev.to_datetime(t.instant);
                let end = this.to_datetime(t.instant);
                ...
                assert!(prev > this);
                let start = this.to_datetime(t.instant);
                let end = prev.to_datetime(t.instant);
...
/// between instants (which is what TZif gives us) and calendar datetimes
/// (which is what we're given as input). It is also necessary that we offset
/// the instant given in TZif at some point, since it is in UTC and the
...
/// input datetime would need to be converted to an instant before searching
/// the transitions.
#[derive(Clone, Debug, Eq, PartialEq)]
enum TransitionWall {
...
/// Interprets the given slice as an unsigned 64-bit big endian integer and
/// returns it.
///
/// # Panics
///
/// When `bytes.len() != 8`.
fn from_be_bytes_u64(bytes: &[u8]) -> u64 {
...
mod tests {
    ...
    use crate::tz::testdata::{TzifTestFile, TZIF_TEST_FILES};
    ...
    fn tzif_to_human_readable(tzif: &Tzif) -> String {
        ...
        if !tzif.leap_seconds.is_empty() {
            ...
            for ls in tzif.leap_seconds.iter() {
                let dt = ls.occurrence.to_datetime();
                let c = ls.correction;
    ...
    fn zoneinfo() {
        ...
        for result in walkdir::WalkDir::new(TZDIR) {
            ...
            if let Err(err) = Tzif::parse(tzname, &bytes) {
