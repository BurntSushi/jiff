/*!
Defines data types shared between `jiff` and `jiff-tz-static`.
...
is that `jiff-tz-static x.y.z` works with one and only one version of Jiff,
...
`TimeZone`. The proc macros in `jiff-tz-static` generate code using these
types (and a few routines).

Secondly, it provides a way to parse TZif data without `jiff-tz-static`
...
of jiff) into the `jiff-tz-static` crate. This can be done automatically with
`jiff-cli`:
...
One alternative is to have `jiff-tz-static` explicitly depend on `jiff` in its
`Cargo.toml`. Then Jiff could expose the parsing routines, as it does here,
and `jiff-tz-static` could use them directly. Unfortunately, this means that
`jiff` cannot depend on `jiff-tz-static`. And that in turn means that `jiff`
cannot re-export the macros. Users will need to explicitly depend on and use
`jiff-tz-static`. Moreover, this could result in some potential surprises
since `jiff-tz-static` will need to have an `=x.y.z` dependency on Jiff for
...
user's `jiff-tz-static = "x'.y'.z'"` line. This is overall annoying and not a
...
that both `jiff` and `jiff-tz-static` depend on. However, the API exposed in
...
generally only expect `jiff-tz-static` to be used in niche circumstances, I
...
When this code is copied to `jiff-tz-static`, the following transformations are
done:

* A header is added to indicate that the copied file is auto-generated.
* All `#[cfg(feature = "alloc")]` annotations are removed. The `jiff-tz-static`
...
this module is copied to `jiff-tz-static`.
*/
