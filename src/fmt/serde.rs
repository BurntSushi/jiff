...
///                 .precision(Some(3));
///
///             // FIXME: Use dedicated printing method.
///             let sdur = jiff::SignedDuration::try_from(*self.0)
///                 .map_err(|_| core::fmt::Error)?;
///             PRINTER
///                 .print_duration(&sdur, StdFmtWrite(f))
///                 .map_err(|_| core::fmt::Error)
///         }
///     }
///
...
pub mod unsigned_duration {
        ...
        pub mod compact {
            ...
            impl<'a> core::fmt::Display for DisplayFriendlyCompact<'a> {
                fn fmt(
                    ...
                    // FIXME: Use dedicated printing method.
                    let sdur = crate::SignedDuration::try_from(*self.0)
                        .map_err(|_| core::fmt::Error)?;
                    PRINTER
                        .print_duration(&sdur, StdFmtWrite(f))
                        .map_err(|_| core::fmt::Error)
    ...
    impl<'a> core::fmt::Display for DisplayISO8601<'a> {
        fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
            ...
            let sdur = crate::SignedDuration::try_from(*self.0)
                .map_err(|_| core::fmt::Error)?;
            PRINTER
                .print_duration(&sdur, crate::fmt::StdFmtWrite(f))
                .map_err(|_| core::fmt::Error)
        }
    }
    ...
    fn parse_iso_or_friendly(
        ...
        // FIXME: Use dedicated parsing routine for `std::time::Duration`.
        let sdur = if first == b'P' || first == b'p' {
            crate::fmt::temporal::DEFAULT_SPAN_PARSER.parse_duration(bytes)
        } else {
            crate::fmt::friendly::DEFAULT_SPAN_PARSER.parse_duration(bytes)
        }?;
        sdur.try_into()
    }
}
