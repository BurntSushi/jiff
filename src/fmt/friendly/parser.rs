...
mod tests {
    ...
    #[test]
    fn scratch() {
        // dbg!(core::mem::size_of::<DesignatorTrie>());
        // dbg!(core::mem::size_of::<
        // TrieNode<{ DESIGNATOR_ALPHABET.len() }, Unit>,
        // >());
        // dbg!(core::mem::size_of::<Unit>());
        // dbg!(core::mem::size_of::<Option<Unit>>());

        dbg!(DESIGNATOR_TRIE.find(b"nanosecondss"));
    }
    ...
    fn parse_span_boundaries() {
        ...
        // For units of hours or smaller, we actually support overflowing
        // the unit values and "rebalancing" since these units are always
        // invariant.
        //
        // BREADCRUMBS: Do we really want to support this? It's kinda
        // necessary for ISO 8601 since that doesn't support non-fractional
        // millis/micros/nanos. But maybe here, we should just refuse to parse
        // it and keep it simpler without any automatic re-balancing? I think
        // the main issue is that otherwise, we can serialize a valid `Span`
        // with fractional units, and then fail to deserialize it. For example,
        // consider serializing `1s2000ms` with fractional unit set to seconds.
        // This will result in `3s`. So in theory, we could overflow. But
        // maybe we just fail to serialize in those cases? Especially since it
        // requires opting into fractional units specifically.
