...
/// # Panics
///
/// This panics if `unit` is not `Hour`, `Minute`, `Second`, `Millisecond`
/// or `Microsecond`.
#[inline(never)]
...
/// Like `fractional_time_to_duration`, but on 128-bit nanoseconds.
///
/// Unlike `fractional_time_to_duration`, this never panics for any unit.
/// Instead, units that aren't hours, minutes, seconds, milliseconds or
/// microseconds will result in an error.
#[inline(never)]
pub(crate) fn fractional_time_to_nanos(
    unit: Unit,
    fraction: t::SubsecNanosecond,
